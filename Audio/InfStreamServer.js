...
let infiniteStream = function(
    ...
    const speechCallback = (stream) => {
        ...
        if (stream.results[0] && stream.results[0].alternatives[0]) {
            tmp = stream.results[0].alternatives[0].transcript;
        }
        ...
        if (stream.results[0].isFinal) {
            ...
            //     tmp =
            //         tmp.substring(0, process.stdout.columns - 4) + '...';
            // }
...
let startServer = ()=>{
    ...
    infiniteStream(opts.encoding.default,
                   opts.sampleRateHertz.default,
                   opts.languageCode.default,
                   opts.streamingLimit.default);
    ...
    app.post('/sentiment', async (req,res) => {
        ...
        const [result] = await client.analyzeSentiment({document: doc});
    ...
    app.get('/', async (req, res) => {
        ...
        const [result] = await langClient.analyzeSentiment({document: doc});
        const sentiment = result.documentSentiment;
